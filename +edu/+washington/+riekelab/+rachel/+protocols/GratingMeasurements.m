classdef GratingMeasurements < manookinlab.protocols.ManookinLabStageProtocol
    properties
        ...
        contrasts = [0.25 0.5 1.0]                              % Grating contrast (0-1)
        orientation = 0.0                           % Grating orientation (deg)
        barWidths = [15, 30, 45, 60, 75]            % Grating bar widths (um)
        temporalFrequencies = [0.5 1.0 2.0]                     % Temporal frequency of modulation (Hz)
    ...
    methods
        ...
        function organizeParameters(obj)
            ...
            [W, C, T] = meshgrid(SFs, obj.contrasts, obj.temporalFrequencies);
            combos = [W(:), C(:), T(:)];
            nConditions = int32(size(combos, 1));
            disp('n_conditions')
            disp(nConditions)
...
%             freqs = freqs(:)';
            ...
            obj.allCombos = zeros(nConditions * int32(obj.numReps), 3);
            for r = 1:obj.numReps
                idx = randperm(nConditions);
                rows = (int32(r)-1)*nConditions + (1:nConditions);
                obj.allCombos(rows, :) = combos(idx, :);
            end
            
            obj.spatialFrequencies = obj.allCombos(:,1)';
        ...
        function prepareEpoch(obj, epoch)
            ...
            obj.temporalFrequency = obj.temporalFreqSeq(obj.numEpochsCompleted+1);
            ...
            % width in Microns
            epoch.addParameter('spatialFrequency', obj.frequency);
