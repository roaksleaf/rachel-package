classdef VariableMeanBars < manookinlab.protocols.ManookinLabStageProtocol
    ...
    methods
        ...
        function prepareRun(obj)
            ...
            if obj.interleave
                deltareps = zeros(size(obj.backgroundRepEpochs));
                minrep = min(obj.backgroundRepEpochs);
                for i=1:length(obj.backgroundRepEpochs)
                    deltareps(i) = obj.backgroundRepEpochs(i)-minrep;
                end
                obj.backgroundFrameDwellsFull = repmat(obj.backgroundFrameDwells, [1, minrep]);
                while any(deltareps)
                    for i=1:length(obj.backgroundRepEpochs)
                        if deltareps(i)>=1
                            obj.backgroundFrameDwellsFull = cat(2, obj.backgroundFrameDwellsFull, obj.backgroundFrameDwells(i));
            ...
            disp(obj.backgroundFrameDwellsFull);
        end
